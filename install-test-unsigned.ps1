...
[CmdletBinding()]
param(
    [switch]$IUnderstandUnsignedTestBuild
)

$ErrorActionPreference = 'Stop'
$root = Split-Path -Parent $PSCommandPath
$version = '1.6.2-R9-HF2-TEST'
...
function Assert-TestModePreconditions
{
    if (-not $IUnderstandUnsignedTestBuild)
    {
        throw 'Instalador de PRUEBA: debes ejecutarlo explicitamente con -IUnderstandUnsignedTestBuild. No se modifico la instalacion actual.'
    }
    ...
    try
    {
        ...
        {
            throw 'Microsoft Defender en tiempo real esta desactivado. El instalador de prueba se niega a continuar.'
        }
    }
    catch
    {
        if ($_.Exception.Message -like '*se niega a continuar*') { throw }
        throw "No se pudo confirmar que Microsoft Defender este activo: $($_.Exception.Message)"
    }

    $manifestPath = Join-Path $publish 'trusted-build-artifacts.json'
    $agentPath = Join-Path $publish 'Agent\GivenX.Agent.exe'
    $uiPath = Join-Path $publish 'UI\GivenX.UI.exe'
    if (-not (Test-Path $manifestPath) -or -not (Test-Path $agentPath) -or -not (Test-Path $uiPath))
    {
        throw 'Este script solo funciona dentro del artifact de GitHub Actions: deben existir Agent, UI y trusted-build-artifacts.json junto a la carpeta Installer.'
    }

    $manifest = Get-Content $manifestPath -Raw | ConvertFrom-Json
    if ([string]$manifest.Version -ne '1.6.2-R9')
    {
        throw "El manifiesto no corresponde a R9. Version encontrada: $($manifest.Version)"
    }
    if ([string]::IsNullOrWhiteSpace([string]$manifest.SourceCommit))
    {
        throw 'El artifact no incluye SourceCommit de GitHub Actions. No se permite una instalacion unsigned de origen no identificado.'
    }
    if ([string]$manifest.SourceCommit -notmatch '^[A-Fa-f0-9]{40}$')
    {
        throw 'SourceCommit del manifiesto tiene un formato invalido.'
    }

    $agentHash = (Get-FileHash $agentPath -Algorithm SHA256).Hash
    $uiHash = (Get-FileHash $uiPath -Algorithm SHA256).Hash
    if ($agentHash -ne ([string]$manifest.AgentSha256).ToUpperInvariant() -or
        $uiHash -ne ([string]$manifest.UiSha256).ToUpperInvariant())
    {
        throw 'Los hashes de Agent/UI no coinciden con trusted-build-artifacts.json. No se modifico la instalacion actual.'
    }

    Write-Host "Artifact de GitHub verificado por hashes. Commit: $($manifest.SourceCommit)" -ForegroundColor Green
    Write-Warning 'MODO DE PRUEBA UNSIGNED: este modo NO sustituye la firma de SignPath y nunca debe usarse para una release publica.'
}
...
try
{
    Write-Host '[1/7] Verificando artifact unsigned de GitHub Actions sin tocar la instalacion actual...' -ForegroundColor Cyan
    Assert-TestModePreconditions
    ...
    {
        throw 'El artifact no contiene ambos ejecutables y su manifiesto de confianza.'
    }
    ...
    $rulesSource = Join-Path $root 'rules'
    if (-not (Test-Path $rulesSource)) { throw 'El artifact de prueba no contiene la carpeta DevTest\rules.' }
    Copy-Item $rulesSource (Join-Path $candidate 'rules') -Recurse -Force
    ...
    Set-Content (Join-Path $candidate 'version.txt') "GivenX Shield $version - NO FIRMADA / SOLO PRUEBAS" -Encoding UTF8
    ...
    Write-Host "GivenX Shield $version fue instalado para PRUEBAS y su arranque quedó comprobado." -ForegroundColor Green
    Write-Warning 'Vuelve a activar Control inteligente de aplicaciones inmediatamente despues de terminar la instalacion/prueba si lo desactivaste temporalmente.'
}
catch
{
    $originalError = $_
